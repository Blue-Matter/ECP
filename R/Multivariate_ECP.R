...
get_pred_sim=function(ECP_obj,OMind,Iind,powind=1){
  ...
  lnams = paste0(dimnames(PPD)[[3]][ds],"_",ys+ECP_obj$First_Yr-1)
  yrs= min(ys+ECP_obj$First_Yr-1):max(ys+ECP_obj$First_Yr-1)
  list(preds=preds,obs=obs,nams=nams,pows=pows,yrs=yrs,lnams=lnams)
}
...
get_pred_sim_yr=function(ECP_obj,OMind,Iind,yind=1:6,powind=1){
  ...
  list(preds=preds,obs=obs,nams=nams,pows=pows,yrs=yrs,lnams2 = lnams)
}
